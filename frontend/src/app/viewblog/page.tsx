...
export default function ViewBlogPage({ searchParams }: { searchParams: { id?: string } }) {
  ...
  if (!blog)
    return (
      <div className="flex justify-center items-center min-h-[40vh] text-gray-500">Không tìm thấy bài viết.</div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-4 relative">
      ...
      <h1 className="text-3xl font-bold text-center text-sky-800 mb-4">{blog.title}</h1>
      <img
      ...
      <div className="mt-10">
        ...
        {/* Danh sách bình luận */}
        {commentLoading ? (
          <div className="text-sky-600 text-center py-6">Đang tải bình luận...</div>
        ) : comments.length === 0 ? (
          <div className="text-gray-500 text-center">Chưa có bình luận nào.</div>
        ) : (
          <div className="space-y-5">
            {comments.map(cmt => (
              <div key={cmt._id} className="bg-white rounded-lg shadow border px-5 py-4">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-semibold text-sky-700">
                    {(typeof cmt.user === "object" && cmt.user?.name) ? cmt.user.name : "Ẩn danh"}
                  </span>
                  <span className="text-gray-400 text-xs">• {cmt.createdAt ? new Date(cmt.createdAt).toLocaleString("vi-VN") : ""}</span>
                </div>
