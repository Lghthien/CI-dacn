...
import { ArrowLeft, CheckCircle2, Loader2, XCircle, Trash2 } from "lucide-react";
import {
  AlertDialog,
  AlertDialogTrigger,
  AlertDialogContent,
...
export default function MyBookingPage() {
  ...
  const handleDelete = async (bookingId: string, onDone?: () => void) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Không thể hủy booking, vui lòng thử lại."); // hoặc custom error dialog
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <h1 className="text-2xl font-bold text-center text-sky-800 mb-4">Đơn Đặt Tour của Tôi</h1>
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-500">Bạn chưa có đơn đặt tour nào.</div>
      ) : (
        <div className="space-y-6">
          {bookings.map((bk) => {
            const tour = typeof bk.tour === "object" && bk.tour !== null ? (bk.tour as Tour) : undefined;
            ...
            return (
              ...
              >
                {/* Nút XÓA booking ở trạng thái "pending" với AlertDialog */}
                {isPending && (
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Bạn muốn hủy đơn đặt tour này?</AlertDialogTitle>
                        <AlertDialogDescription>
                          Thao tác này không thể hoàn tác.<br />
                          Bạn chắc chắn muốn xóa đơn đặt tour: <b>{tour?.title}</b>?
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                          ...
                          onClick={(e) => {
                            // Phải dùng e.preventDefault để ngăn dialog tự đóng, và truyền hàm đóng vào handleDelete
                            e.preventDefault();
                            handleDelete(bk._id, () => {
                              // đóng dialog
                        ...
                        >
                          {deleting === bk._id ? <Loader2 className="animate-spin" size={18} /> : "Xác nhận hủy"}
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                ...
                <img
                  ...
                  onError={e => ((e.target as HTMLImageElement).src = "/images/default_tour.jpg")}
                />
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between">
                      ...
                      >
                        ...
                        {bk.status === "pending" && <Loader2 size={16} className="animate-spin" />}
                    ...
                    <div className="text-gray-500 mb-2">{tour?.location || "Không rõ địa điểm"}</div>
                    <div className="text-gray-700 text-sm mb-2 line-clamp-3">{tour?.description || ""}</div>
                  </div>
                  ...
                  <div className="text-gray-400 text-xs mt-2">
                    Đặt ngày: {bk.createdAt ? new Date(bk.createdAt).toLocaleString("vi-VN") : ""}
                  </div>
                </div>
              </div>
            );
