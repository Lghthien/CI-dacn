...
export default function BlogPostPage() {
  ...
  // Handle image upload (fake, bạn chỉnh endpoint upload nếu có backend riêng)
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Nếu backend có endpoint upload ảnh riêng, upload ở đây rồi lấy url đưa vào form.image
    // Ví dụ backend trả về { url: "..." }
    // Nếu không có thì bỏ qua phần này, chỉ lưu link external hoặc lưu trực tiếp file base64/text
    // Ở đây demo upload tới /blog-post/image (bạn sửa cho đúng backend nếu cần)
    ...
    <div className="min-h-screen bg-gradient-to-r from-sky-50 via-pink-50 to-blue-50 flex justify-center items-center py-10">
      <Card className="w-full max-w-xl rounded-3xl shadow-2xl border-sky-100 bg-white/95">
        <CardContent className="p-8">
          <h2 className="text-2xl font-bold text-center text-sky-800 mb-6">Đăng Blog Mới</h2>
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col items-center">
              <label className="cursor-pointer">
                <span className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs text-sky-700 underline">Chọn ảnh đại diện</span>
                </span>
                <input
                  type="file"
                  accept="image/*"
            ...
            <Button
              ...
              disabled={submitting}
            >
              {submitting ? <Loader2 size={20} className="animate-spin mr-1" /> : null}
              Đăng Blog
            </Button>
          </form>
        </CardContent>
