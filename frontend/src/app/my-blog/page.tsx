...
export default function MyBlogPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/blog-post`)
      .then(res => res.json())
      .then(data => {
        // Lọc blog của user này
        setBlogs(data.filter((b: any) => b.author === userId || b.author?._id === userId));
      })
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <h1 className="text-2xl font-bold text-sky-800 mb-2 text-center">Blog của tôi</h1>
      {message && (
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {blogs.map(blog => (
            <div key={blog._id} className="rounded-xl shadow-lg border bg-white flex flex-col md:flex-row gap-4 p-4 relative">
              <img
                src={blog.image || "/images/default_blog.jpg"}
                alt={blog.title}
                className="rounded-xl w-full md:w-52 h-40 object-cover border"
                onError={e => ((e.target as HTMLImageElement).src = "/images/default_blog.jpg")}
              />
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  <h2 className="text-xl font-bold text-sky-800">{blog.title}</h2>
                  <div className="text-gray-700 text-base line-clamp-3 mt-2">{blog.content}</div>
                </div>
                <div className="flex gap-3 mt-4">
                  {/* Edit */}
                  <Dialog open={editBlog?._id === blog._id} onOpenChange={open => open ? openEditModal(blog) : setEditBlog(null)}>
                    <DialogTrigger asChild>
                      <Button size="sm" className="bg-yellow-400 text-sky-900 font-semibold rounded-xl hover:bg-yellow-500 flex items-center gap-1 px-4 py-2">
                        <Pencil size={16} /> Sửa
                      </Button>
                    </DialogTrigger>
                    {editBlog?._id === blog._id && (
                      <DialogContent className="max-w-xl">
                          ...
                          <div className="flex flex-col items-center">
                            <label className="cursor-pointer">
                              <span className="flex flex-col items-center gap-2">
                                ...
                                <span className="text-xs text-sky-700 underline">Chọn ảnh mới</span>
                              </span>
                              <input
                                type="file"
                                accept="image/*"
                        ...
                        <DialogFooter>
                          <Button onClick={handleSaveEdit} disabled={saving} className="bg-sky-700 text-white rounded-xl">
                            {saving ? <Loader2 size={18} className="animate-spin" /> : "Lưu"}
                          </Button>
                        </DialogFooter>
                      </DialogContent>
                    )}
                  ...
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button size="sm" variant="destructive" className="rounded-xl px-4 py-2 flex items-center gap-1">
                        <Trash2 size={16} /> Xóa
                      </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Bạn chắc chắn muốn xóa blog này?</AlertDialogTitle>
                        <AlertDialogDescription>
                          Thao tác này không thể hoàn tác.<br />Xóa blog: <b>{blog.title}</b>?
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        ...
                        >
                          {deletingId === blog._id ? <Loader2 className="animate-spin" size={16} /> : "Xác nhận"}
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                ...
                <div className="text-gray-400 text-xs mt-2">
                  Đăng lúc: {blog.createdAt ? new Date(blog.createdAt).toLocaleString("vi-VN") : ""}
                  {blog.updatedAt && (
                    <> <br />Cập nhật: {new Date(blog.updatedAt).toLocaleString("vi-VN")}</>
                  )}
                </div>
              </div>
            </div>
